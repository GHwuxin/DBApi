...
axios.interceptors.request.use((config) => {
  ...
  if (
    config.method === "post" &&
    config.headers["Content-Type"] === CONTENT_TYPE.FORM_URLENCODED
  ) {
...
axios.interceptors.response.use(
  ...
  (error) => {
    if (error.response.status == "401") {
      // 不是api请求测试的请求，就跳转登录页
      if (
        !error.response.config.url.startsWith("http://") &&
        !error.response.config.url.startsWith("https://")
      ) {
        router.push("/login");
      } else {

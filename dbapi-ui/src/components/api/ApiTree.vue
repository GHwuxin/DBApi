<template>
  <div class="tree_root">
    <el-button type="primary"
               ...
               size="mini"
               plain>{{ $t('m.create_group') }}</el-button>
    ...
    <el-tree :data="data"
      ...
      <span class="custom-tree-node"
        ...
        <span class="left">
          ...
          <span class="el-icon-folder-opened title"
                v-if="data.type=='group'"
                :title="node.label"> {{ node.label }}</span>
          <span class="iconfont icon-api title"
                :class="{disable:data.status === 0}"
                v-if="data.type=='api'"
                :title="node.label"> {{ node.label }}</span>
        </span>
              ...
              <el-dropdown-item>
                <el-tooltip class="item"
                  ...
                  <i class="el-icon-edit"
                     @click="$router.push({path: '/api/edit', query: {id: data.id}});"></i>
                </el-tooltip>
              </el-dropdown-item>
        ...
        <span class="align:right"
          ...
          <el-tooltip :open-delay="500"
                      class="item"
                      effect="light"
                      :content="$t('m.create_api')"
                      placement="top">
            <i class="el-icon-circle-plus"
               @click="$router.push({path: '/api/add', query: {groupId: data.id}});"></i>
          </el-tooltip>
          <el-dropdown size="medium">
            <i class="el-icon-more-outline"></i>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>
                <el-tooltip class="item"
                            ...
                            placement="left">
                  <i class="el-icon-edit"
                     @click="editGroupDialog=true;handleEditGroup(data);"></i>
                </el-tooltip>
              </el-dropdown-item>
    ...
    <el-dialog :title="$t('m.create_group')"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="createDialog = false;createGroup()">{{ $t('m.save') }}</el-button>
      </span>
    </el-dialog>

    <el-dialog :title="$t('m.edit_group')"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="editGroupDialog = false;editGroup()">{{ $t('m.save') }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createGroup() {
      this.axios
        .post('/group/create/', { name: this.groupName })
        .then((response) => {
          this.getAllApiTree()
        })
        .catch((error) => {

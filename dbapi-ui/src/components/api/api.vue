<template>
  ...
  <div class="api_root">
    ...
    <div class="api">
        ...
        <div class="left">
          <div class="search">
            <div>
              ...
              <el-input v-model="keyword.name"
                        style="width:150px;"
                        clearable
                        size="mini"></el-input>
              <span class="label">{{ $t('m.note') }}</span>
              <el-input v-model="keyword.note"
                        style="width:150px;"
                        clearable
                        size="mini"></el-input>
              <span class="label">{{ $t('m.path') }}</span>
              <el-input v-model="keyword.path"
                        style="width:200px;"
                        clearable
                        size="mini">
              ...
              <el-button type="primary"
                         ...
                         @click="search"
                         plain
                         size="mini">{{ $t('m.search') }}</el-button>
            </div>
          ...
          <el-dropdown @command=""
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="item">
                <span class="iconfont icon-document"
                      @click="dialogVisible2=true">{{ $t('m.export_api_doc') }}</span>
              </el-dropdown-item>
              <el-dropdown-item command="item">
                <span class="el-icon-download"
                      @click="dialogVisible3=true">{{ $t('m.export_api') }}</span>
              </el-dropdown-item>
              <el-dropdown-item command="item">
                <el-upload action="/apiConfig/import"
                           accept=".json"
              ...
              <el-dropdown-item command="item">
                <span class="iconfont icon-group"
                      @click="dialogVisible4=true">{{ $t('m.export_api_groups') }}</span>
              </el-dropdown-item>

              <el-dropdown-item command="item">
                <el-upload action="/apiConfig/importGroup"
                           accept=".json"
        ...
        <el-table :data="tableData"
          ...
          <el-table-column :label="$t('m.name')">
            <template slot-scope="scope">
              <i class="iconfont icon-on_line1 circle"
                 v-if="scope.row.status == 1"></i>
              <i class="iconfont icon-off_line circle offline"
                 v-else></i>
              <i class="el-icon-lock circle lock"
                 v-if="scope.row.access == PRIVILEGE.PRIVATE"></i>
              <i class="el-icon-unlock circle "
                 v-else></i>
          ...
          <el-table-column :label="$t('m.note')"
                           prop="note">
          </el-table-column>
          <el-table-column label="Content-Type"
                           prop="contentType"
                           sortable></el-table-column>
          <el-table-column :label="$t('m.parameters')">
            <template slot-scope="scope">
              <div v-show="scope.row.contentType === CONTENT_TYPE.FORM_URLENCODED ">
                <data-tag v-for="item in scope.row.p"
                          :name="item.name"
                          :type="item.type"></data-tag>
              </div>
      ...
      <el-dialog :title="$t('m.export_api_doc')"
        ...
        <span slot="footer"
          ...
          <el-button type="primary"
                     @click="dialogVisible2 = false;exportDocs()">{{ $t('m.export') }}</el-button>
        </span>
      </el-dialog>

      <el-dialog :title="$t('m.export_api')"
        ...
        <el-tree :data="treeData"
                 ...
                 ref="tree2"></el-tree>
        <span slot="footer"
              class="dialog-footer"><el-button @click="dialogVisible3 = false">{{ $t('m.cancel') }}</el-button>
          <el-button type="primary"
                     @click="dialogVisible3 = false;exportConfig()">{{ $t('m.export') }}</el-button>
        </span>
      </el-dialog>

      <el-dialog :title="$t('m.export_api_groups')"
        ...
        <span slot="footer"
          ...
          <el-button type="primary"
                     @click="dialogVisible4 = false;exportGroupConfig()">{{ $t('m.export') }}</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
...
.api_root {
  ...
  .api {
    ...
    .tool {
      box-shadow: 0px 0px 3px 2px #2187a8;
